...
getData().then ( value => {
  ...
  fs.writeFile(`./json/${value[0].currentMonth}_tariffs.json`, JSON.stringify(value), err => {
    if (err) throw err;
  ...
  fs.appendFile('./tariffs.csv', `${value[0].currentMonth}`, err => {
    if (err) throw err;
  ...
  for ( let i = 0; i < value.length; i++) {
    fs.appendFile('./tariffs.csv', newLine(), err => {
      if (err) throw err;
    });
